#### About this script ####
## Title: Probability of causation of lung cancer due to asbestos: logistic 
##        model for the duration of exposure to asbestos
##
## Purpose: Plot of the probability of causation (PoC) of lung cancer due to
## asbestos (duration of exposuire), under three scenarios: 
## 1. no misclassification error assumed, 2. misclassification error with a 
## factor of 1.5 and 3. misclassification error with a factor of 2. 
## Calculations are based on SYNERGY data and adjusted for confounding 
## due to age, sex, smoking, and center. 
##
...
#### Create plot #### 
logistic_PoC_plot <- ggplot(df.dur0, aes(x = asbestos_dur0)) +
...
ggsave(
  logistic_PoC_plot,
  filename = file.path(figfolder, "PoC_dur0_logistic.pdf"),
