...
affiliations:
  ...
  - id: lexces
    ...
    country: the Netherlands
date: today
abstract: "This report contains the main results from the SYNERGY pooled case-control study meta-analysis to estimate the exposure-response relation of lung cancer risk and cumulative exposure to asbestos. Three approaches are compared: univariate and bivariate two-stage individual participant data (IPD) meta-analysis, and one-stage IPD meta-analysis. Exposure-response relations are used to estimate the exposure threshold at which the likelihood of lung cancer is doubled, corresponding to a probability of causation of 0.5. The upper 95% prediction interval is used as to obtain the exposure thresholds at which it is presumably plausible that lung cancer was caused by asbestos."
execute: 
  echo: false
  warning: false
format:
...
conflicts_prefer(dplyr::filter, dplyr::select, table1::label)
```
...
Exposure to asbestos as continuous variable: Lifetime cumulative exposure to asbestos (**`asbestos_cum0`**)
...
source("scripts/two_stage_metanalysis.R")
```

![Two-stage univariate individual participant data (IPD) meta-analysis of all studies included in the pooled case-control SYNERGY study. INCO included multiple centers in different countries, reason why each country is presented as an individual study.](images/Figure_S2.png){fig-align="center"}
...
A model with full random effects did not converge status \~ (1 + asbestos_cum0 + agegroup + sex + packyrs + time_quit \| study_name), therefore alternative models
...
saveRDS(mixed_model_cum0, file = paste0(tempfolder,"/one-stage_selective-random_cum0_main.rds"))
```
...
beta_coefficients %>% 
  write.table(., paste0(tabfolder, "/PoC_one-stage-main_count_plus50.txt"), sep = "\t")
```
